...
class DependencyParserModel(object):
    ...
    def build_graph(self, graph, embedding_array, Config):
        ...
        with graph.as_default():
           ...
           # This is for fixing embedding size
           ...
           # self.app = optimizer.apply_gradients(grads)  without gradient clipping
    ...
    def forward_pass(self, embed, weights_input, biases_input, weights_output):
        ...
        product1 = tf.matmul(weights_input, embed, transpose_b=True)

        temp_sum = product1 + biases_input
        h = tf.pow(temp_sum, 3)
        # Uncomment to change activation function to sigmoid
        # temp_value = tf.nn.sigmoid(h2)

        # Uncomment to change activation funtion to tanh
        # temp_value = tf.tanh(h2)

        # Uncomment to change activation function to ReLu
        # temp_value = tf.nn.relu(h2)

        final = tf.matmul(weights_output, h)
        finalaftertranspose = tf.transpose(final)
        return finalaftertranspose
