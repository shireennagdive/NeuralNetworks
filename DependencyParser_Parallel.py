...
class DependencyParserModel(object):
    ...
    def build_graph(self, graph, embedding_array, Config):
        ...
        with graph.as_default():
            ...
            weights_input = tf.Variable(
                tf.truncated_normal(shape=[Config.hidden_size, Config.n_Tokens * Config.embedding_size],
                                    stddev=0.1))
            # computed weights output
            weights_output = tf.Variable(
                tf.truncated_normal(shape=[parsing_system.numTransitions(), Config.hidden_size], stddev=0.1))

            # initialized bias_output
            bias_input = tf.Variable(tf.zeros([Config.hidden_size, 1]))

            # called forward pass and computed output of first layer which is final output
            self.train_prediction = self.forward_pass(embeddings, weights_input, bias_input, weights_output)
            ...
            l2_loss_input = Config.lam * tf.nn.l2_loss(weights_input)
            l2_loss_output = Config.lam * tf.nn.l2_loss(weights_output)
            l2_loss_bias = Config.lam * tf.nn.l2_loss(bias_input)
            l2_loss_embed = Config.lam * tf.nn.l2_loss(embeddings)

            total_ce_l2_loss = ce_loss + l2_loss_input + l2_loss_output + l2_loss_bias + l2_loss_embed
            ...
            self.test_pred = self.forward_pass(test_embed, weights_input, bias_input, weights_output)
    ...
    def forward_pass(self, embed, weights_input, biases_input, weights_output):
        ...
        =======================================================
        """
        #SPlitting input weights and embeddings
        weights_input_1 = weights_input[:, :900]
        weights_input_2 = weights_input[:, 900:1800]
        weights_input_3 = weights_input[:, 1800:]
        embed_1 = embed[:, :900]
        embed_2 = embed[:, 900:1800]
        embed_3 = embed[:, 1800:]
        input_layer = tf.pow(tf.matmul(weights_input_1, tf.transpose(embed_1)) + biases_input, 3) + tf.pow(
            tf.matmul(weights_input_2, tf.transpose(embed_2)) + biases_input, 3) + tf.pow(
            tf.matmul(weights_input_3, tf.transpose(embed_3)) + biases_input, 3)
        hidden = input_layer

        final = tf.matmul(weights_output, hidden)
